...
ipcMain.on("getDeliveryData", async (event, filter) => {
  ...
  if (allItems && (allItems == null ? void 0 : allItems.length)) {
    ...
    const mergedData = allItems[0].data.map((delivery2) => {
      ...
      let color = "";
      if (remainDays >= 0 && delivery2.status == "Item Delivered") {
        color = "green";
      } else if (remainDays < 0 && remainDays > -2 && delivery2.status != "Item Delivered") {
        color = "orange";
      } else if (remainDays < -1 && delivery2.status != "Item Delivered") {
        color = "red";
      } else if (remainDays <= 0 && delivery2.dest_ofc_id != delivery2.office_id && delivery2.status != "Item Delivered") {
        color = "yellow";
      }
      return {
        ...delivery2,
